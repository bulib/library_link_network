...
alma_host = os.environ.get('ALMA_HOST')
alma_user = os.environ.get('ALMA_USER')
alma_pwd = os.environ.get('ALMA_PWD')

# multi threaded version

def check_completed():
    files = glob.glob('publish/*.gz')
    return(len(files)>0)

def get_files():
    os.chdir('/usr3/bustaff/jwa/Zepheira')
    ...
    for d in dir_list:
        ...
        try:
            ...
            print()
    ## now we delete the BibWorks logfiles
    dir_list = glob.glob("BibWorks*")
    for d in dir_list:
        print(d)
        try:
            shutil.rmtree(d)
    ...
    username = alma_user
    password = alma_pwd
    #print(alma_host,alma_user,alma_pwd)
    with pysftp.Connection(hostname,username=username,password=password) as sftp:
        files = sftp.listdir('alma/Zepheira')
        if len(files) < 15:
            return('no files yet')
        try:
...
def copy_db(number):
    wikidata = Path("wikidata.db.base")
    if not wikidata.is_file():
        return('no database found')
    for i in range(1,num+1):
        copy_name = 'wikidata' + str(i) + '.db'
        response = call(["cp", "wikidata.db.base", copy_name])
        if response != 0:
...
def submit_job(number):
    for i in range(1,num+1):
        ...
        if not wikidata.is_file():
            return('no database found')
        response = call(["/usr/local/bin/qsub", "./enhance_bib_records_with_linked_data_sources.py", str(i)])
        print('submit job:' , number, response)
        if response != 0:
...
if __name__ == '__main__':
    ## first check to see if it is already completed
    if not check_completed():
        ...
        if response == 'success':
            resp = copy_db(num)
            resp = submit_job(num)
            resp = call(["/usr/local/bin/qsub", "HarvestOpenBUreccords.py"])
        else:
            print('No files to harvest yet')
    else:
